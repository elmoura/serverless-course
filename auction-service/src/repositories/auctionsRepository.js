...
export class AuctionsRepository {
  ...
  async findById(id) {
    try {
      const result = await this.dynamoDbConnection.get({
  ...
  async create(auction) {
    try {
      return this.dynamoDbConnection.put({
        TableName: process.env.AUCTIONS_TABLE_NAME,
        Item: auction
      }).promise();
    } catch (error) {
  ...
  async placeBid(auctionId, bidAmount) {
    try {
      ...
      const updateResult = await this.dynamoDbConnection.update(params).promise();
  ...
  async getEndedAuctions() {
    ...
    const result = await this.dynamoDbConnection.query(findExpiredOpenAuctionsParams).promise();
    return result.Items;
  };
